	...
	class Flippercode_Product_Overview {
		...
		function __construct($pluginInfo) {
			...
			if(isset($pluginInfo['is_premium']) && $pluginInfo['is_premium'] == 'false'){
				...
				$this->commonBlocks = array( 'product-support','suggestion-area','newsletter');
		...
		function setup_plugin_info($pluginInfo) {
			...
			delete_option($this->productTextDomain.'_user_has_license');
		    $this->userHasLicense = get_option( $this->productTextDomain.'_user_has_license' );
		    $this->loadPromotionalProducts = get_option( $this->productTextDomain.'_hide_promotional_products' );
		    $this->licenseDetails = get_option( $this->productTextDomain.'_license_details' );
