<?php
/**
 * Generate Frontend Form and it's Elements.
 * @author Flipper Code <hello@flippercode.com>
 * @version 1.0.0
 * @package Forms
 */

if ( ! class_exists( 'FlipperCode_Frontend_Markup' ) ) {
	 ...
	 * @package Maps
	 */
	class FlipperCode_Frontend_Markup {
		/**
		 * Form Title
		 * @var String
		 */
		 ...
		 * @var String
		 */
		public $form_type = 'fc-form-horizontal';
		/**
		 * Call to Action Slug
		 * @var String
		 */
		 ...
		 * @var Array
		 */
		private $form_elements = array( 'extensions','text','checkbox','radio','submit','button','select', 'hidden', 'wp_editor', 'html', 'datalist','textarea' , 'file' , 'div' , 'blockquote','html' , 'image','group','table', 'message', 'anchor', 'image_picker' );
		/**
		 * Attributes Allowed
		 * @var Array
		 */
		...
		/**
		 * Add applicatable filters in the filters array.
		 * @param [type] $filter_name [description]
		 */
		public function add_extension_fields($filter_name) {
			array_push($this->filters, $filter_name);
		}
		/**
		 * Form Method
		...
		public function get_form_messages() {
			...
			if ( $response['error'] ) {
				...
				$output .= '<div class="fc-error">';
				$output .= ''.$response['error'].'</div>';
			} else {

				$output .= '<div class="fc-success">';
		...
		public static function get_element_html( $name, $type, $atts ) {
			...
			if ( 'hidden' == $type ) {
				...
				if ( ! empty( $atts['lable'] ) ) {
				$element_output .= apply_filters( 'wpgmp_input_label_'.$name, '<div class="fc-col-md-3"><label for="'.$name.'">'.$atts['lable'].'</label>'.self::element_mandatory( @$atts['required'] ).'</div>' ); }
				$element_output .= @$atts['before'] ? @$atts['before'] : '<div class="fc-col-md-8">';
				$element_output .= call_user_func( 'FlipperCode_HTML_Markup::field_'.$type, $name, $atts );
		...
		public static function field_image_picker($name, $atts) {
			...
			$html = FlipperCode_HTML_Markup::field_image('selected_image', array(
				...
				'src' => $atts['src'],
				'width' => '32',
				'class' => 'noclass selected_image col-md-1'.$class,
				'height' => '37',
		...
		public static function button($title, $link) {
			...
			return '<span class="fc-btn"><a href="'.esc_html( $link ).'">'.__( $title,WPGMP_TEXT_DOMAIN ).'</a></span>';
		}
