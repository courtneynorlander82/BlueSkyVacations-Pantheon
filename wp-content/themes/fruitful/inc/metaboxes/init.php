...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		$scripts = array( 'jquery', 'jquery-ui-core', 'cmb-datepicker', /*'media-upload', */'cmb-timepicker', 'cmb-check', 'cmb-select2', 'cmb-custom' );
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'fruitful'),
					'defaultString' => __( 'Default', 'fruitful'),
					'pick'          => __( 'Select Color', 'fruitful'),
					'current'       => __( 'Current Color', 'fruitful'),
				) );
			}
		} else {
		...
		wp_register_script( 'cmb-check', 	  CMB_META_BOX_URL . 'js/check/icheck.min.js' );
		wp_register_script( 'cmb-select2', 	  CMB_META_BOX_URL . 'js/select2/select2.min.js' );
		wp_register_script( 'cmb-scripts', 	  CMB_META_BOX_URL . 'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		wp_register_script( 'cmb-custom', 	  CMB_META_BOX_URL . 'js/custom.js' );
		...
		wp_register_style( 'cmb-select2',  CMB_META_BOX_URL . 'js/select2/select2.css');
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( $this->_meta_box['cmb_styles'] ) {
				wp_enqueue_style( 'cmb-select2');
				wp_enqueue_style( 'cmb-check' );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'fruitful') );
